...
export const InteractiveShader = ({
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      // Smooth value noise (quintic-interpolated) — removes the blocky pixelation.
      float noise(vec2 x) {
          vec2 i = floor(x);
          vec2 f = fract(x);
          float a = hash(i);
      ...
      void main() {
        ...
        for (int i=0; i<MARCH_STEPS; i++) {
            ...
            float density = map(p);
            if (density > 0.0) {
            float shade = 0.35 + 0.65 * density;
            vec3 monoColor = vec3(shade);   // grayscale
            col += monoColor * density * 0.12 * uColorIntensity;
            }
            t += 0.1;
        }

        // Dither to remove colour banding (highest quality)
    ...
    const resizeCanvas = () => {
      // Supersample: render above CSS resolution (min 1.75x) so the browser
      // downsamples it for smooth, anti-aliased pixels on any display.
      const dpr = Math.min(Math.max(window.devicePixelRatio || 1, 1.75), 2.5);
